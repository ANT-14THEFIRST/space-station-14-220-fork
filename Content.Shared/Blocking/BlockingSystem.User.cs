...
public sealed partial class BlockingSystem
{
    ...
    {
        foreach (var item in component.BlockingItemsShields)
        {
            if (TryComp<BlockingComponent>(item, out var blockComp) && blockComp.IsBlocking)
            StopBlocking(item, blockComp, uid);
        }
    }
}
